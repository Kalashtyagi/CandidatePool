...
const OfferLetters = () => {
  ...
  const getAssigneeData = async () => {
    ...
    if (jobId !== "" || null || undefined) {
      ...
      if (response) {
        const int_Clear = response?.data?.filter((item) => 
          {
            return item?.status?.toLowerCase() === 'interview round 2-clear'.toLowerCase();
          }
              ...
              <Select
                ...
                options={
                  ...
                  listForm?.map((options) => ({
                    value: options?.pid,
                    label: options?.pid,
                    jobrecruitmentData: options?.job_recruitment,
                  }))
                }
                onChange={(e) => {
