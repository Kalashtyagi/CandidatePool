...
const EditViewLeads = () => {
  ...
  useEffect(() => {
    if (state?.MrfStatus === 1) {
      setMrfStatus('Open');
    } else if (state?.MrfStatus === 2) {
      setMrfStatus('Closed');
    }
    // Add more conditions if there are more statuses
  }, [state]);

  const handleMrfStatusChange = (e) => {
  ...
  const showData = [
    ...
    },
    {
      ...
      data: (
        ...
        id="mrfStatus"
        className="border w-full px-2 py-2"
        onChange={handleMrfStatusChange}
        value={mrfStatus === 'Open' ? '1' : mrfStatus === 'Closed' ? '2' : ''}
      >
  ...
  return (
    <div>
      ...
      <BackButton route={-1}/>
        ...
        <h2 className="text-xl px-2 border-gray-300">MRF</h2>
