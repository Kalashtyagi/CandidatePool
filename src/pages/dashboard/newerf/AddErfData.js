...
console.log("titleData",titleData)
  ...
  const postMrfData=async()=>{ 
   ...
   if (response?.code === 200) {
     toast.success('Lead Generate Successfully');
...
let titleId;
        ...
        <div className="relative my-6 mx-2 w-2/3">
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none h-[95vh]">
            ...
            <div className="flex items-start justify-between lg:p-5 p-4 border-b border-solid border-slate-200 rounded-t">
              <h3 className="text-3xl font-semibold">Add MRF</h3>
              <button
  ...
  >
    ...
    {titleData.length > 0 && titleData.map((list) => (
      <option key={list.id}>
        {list.titel}
      </option>
    ))}
                ...
                <div className=" col-span-4  md:pr-2 md:py-0 py-1">
                  ...
                  <Select
                   ...
                   isClearable
                    // value={skill}
                    // options={allHodData?.map((skills) => ({
                    //   value: skills?.id,
                    //   label: skills?.name,
                    // }))}
                    // onChange={(e) => setIsHod(e.map((x) => x.value))}
                    // isMulti={false}
                    // isClearable
                  />
                ...
                <div className=" col-span-4 md:pr-2 md:py-0 py-1">
                  ...
                  <Select
                   ...
                   isClearable
                    // value={skill}
                    // options={allProjectLead?.map((skills) => ({
                    //   value: skills?.id,
                    //   label: skills?.name,
                    // }))}
                    // onChange={(e) => setIsProjectLead(e.map((x) => x.value))}
                    // isMulti={false}
                    // isClearable
                  />
