...
const AddFeedback = () => {
  ...
  const handleChange = (selectedOption) => {
    ...
    setCandidatedetails({
      ...
      highest_qualification: singleData.qualifications_id,
  ...
  const getQualificationName = async() =>{
    if(candidatedetails.highest_qualification!=''){
    try{
      const response =   await axios.get(`${process.env.REACT_APP_API_URL}qualifications`)
      const result = await response?.data?.data;
      console.log("QName",result)
      const QName = result.find((item)=>item.id == candidatedetails.highest_qualification)
      console.log("qqqqq",QName)
      setQualName(QName.name)
    }
   catch(error){
    console.log(error);
   }
  ...
  const handleSubmit = (e) => {
    ...
    axios
      ...
      .catch((err) => {
        console.log("messa",err.response.data.message.interviewround);
        toast.error(err.response.data.message.interviewround)
      });
  };
  ...
  console.log("ccc",candidatedetails)
  return (
    <>
