...
export default function CandidatePCC() {
...
useEffect(()=>{
  ...
  const searchCandidates = async () => {
    ...
    try {
      ...
      const [apiResponse13,apiResponse25,apiResponse27,apiResponse28] = await Promise.all([
        fetchData(13),
        // fetchData(23),
        // fetchData(24),
        fetchData(25),
      ...
      const combinedCandidates = [...apiResponse13,...apiResponse25,...apiResponse27,...apiResponse28];
  ...
  const handleSendUplaodPCC = async (id) => {
    try {
      setLoading(true);
      const request = await axios.get(
        `${process.env.REACT_APP_API_URL}pccdocumentslink/${id}`,
        {
          headers: {
            Authorization: `${authorize}`,
    ...
    },
    {
      ...
      selector: (row) => (
        <div>
            ...
            <button onClick={()=>handleSendUplaodPCC(row.Id)}>
    ...
    <div>
      ...
      <div className="min-h-screen p-6 bg-gradient-to-r from-blue-100 to-blue-200">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold mb-4 text-gray-800"> Candidate PCC </h1>
        </div>
